...
2. Reconstruct the network for a variable sized image
3. Train function: Continue training on top of a model or train from scratch
4. Predict function
5. Load more models
...
if __name__ == "__main__":
    ...
    with sess.as_default():
        ...
        if "pspnet50" in args.model:
            pspnet = PSPNet50(nb_classes=150, input_shape=(None, None),
                              weights=args.model)
        elif "pspnet101" in args.model:
            if "cityscapes" in args.model:
                pspnet = PSPNet101(nb_classes=19, input_shape=(None, None),
                                   weights=args.model)
            if "voc2012" in args.model:
                pspnet = PSPNet101(nb_classes=21, input_shape=(None, None),
                                   weights=args.model)
        ...
        #     print("     BF Predict Model", str(datetime.now()), datetime.now() - TIME_START)
